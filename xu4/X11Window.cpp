...
#if 1
	// Fullscreen
	Atom wm_state = XInternAtom(x11Display, "_NET_WM_STATE", 0);
	Atom fullscreen = XInternAtom(x11Display, "_NET_WM_STATE_FULLSCREEN", 0);
...
void X11Window::HideMouse()
{
	...
	Pixmap pixmap = XCreateBitmapFromData(x11Display, xwin, bitmap, 8, 8);

	XColor black = { 0 };
	Cursor cursor = XCreatePixmapCursor(x11Display,
		pixmap,
		pixmap,
		&black,
		&black,
	...
	XDefineCursor(x11Display, xwin, cursor);
